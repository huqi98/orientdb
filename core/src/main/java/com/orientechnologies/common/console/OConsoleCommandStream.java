...
public class OConsoleCommandStream implements OCommandStream {
  ...
  private enum Symbol {
    ...
    DOUBLE_QUOTE, SINGLE_QUOTE, //quotes

    HYPHEN, POUND, //single row comments

    SLASH, ASTERISK, //multi-row comments
    ...
    try {
      ...
      while (true) {
        ...
        case SLASH://found a slash, if there is an asterisk it is a multi-line comment
          ...
          break;
        case CLOSING_ASTERISK: // you are in a multi-line comment and found an asterisk, if there is a slash it's closing the comment
          if (symbol == Symbol.EOF) {
            return result.toString().trim();
          }
