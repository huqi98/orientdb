...
/**
 * <p>
 * Checks if a record can be safely deleted (throws OCommandExecutionException in case). A record cannot be safely deleted if it's a
 * vertex or an edge (it requires additional operations).</p>
 * <p>
 * The result set returned by syncPull() throws an OCommandExecutionException as soon as it finds a record that cannot be safely
 * deleted (eg. a vertex or an edge)</p>
