...
public abstract class OSQLFunctionAbstract implements OSQLFunction {
  ...
  /**
   * Attempt to extract a single item from object if it's a multi value {@link OMultiValue} If source is a multi value
   *
   * @param source a value to attempt extract single value from it
   * @return If source is not a multi value, it will return source as is. If it is, it will return the single element in it. If
   * source is a multi value with more than 1 element null is returned, indicating an error
   */
   ...
   * @param requireSingleProperty True if the method should return null when source doesn't have a single property. Otherwise, the
   *                              object will be returned.
   * @return If source is a map-like object with single property, that property will be returned If source is a map-like object with
   * multiple properties and requireSingleProperty is true, null is returned indicating an error If source is not a map-like object,
   * it is returned
   */
