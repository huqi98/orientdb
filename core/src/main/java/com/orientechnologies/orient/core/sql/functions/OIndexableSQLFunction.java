...
public interface OIndexableSQLFunction extends OSQLFunction {
   ...
   * @param operator   the operator after the function, eg. in <code>select from Foo where myFunction(name) &gt; 4</code> the
   *                   operator is &gt;
   * @param rightValue the value that has to be compared to the function result, eg. in <code>select from Foo where myFunction(name)
   *                   &gt; 4</code> the right value is 4
   * @param ctx        the command context for this query
   * @param args       the function arguments, eg. in <code>select from Foo where myFunction(name) &gt; 4</code> the arguments are
   *                   [name]
   ...
   * @param args       the function arguments, eg. in <code>select from Foo where myFunction(name) &gt; 4</code> the arguments are
   *                   [name]
   * @return an estimantion of how many entries will be returned by searchFromTarget() with these parameters, -1 if the estimation
   * cannot be done
   */
  ...
  /**
   * checks if the function can be used even on single records, not as an indexed function (even if the index does not exist at
   * all)
   *
   * @param target     the query target
   * @param operator   the operator after the function, eg. in <code>select from Foo where myFunction(name) &gt; 4</code> the
  ...
  /**
   * Checks if this function can be used to fetch data from this target and with these arguments (eg. if the index exists on this
   * target and it's defined on these fields)
   *
   * @param target     the query target
   * @param operator   the operator after the function, eg. in <code>select from Foo where myFunction(name) &gt; 4</code> the
   *                   operator is &gt;
