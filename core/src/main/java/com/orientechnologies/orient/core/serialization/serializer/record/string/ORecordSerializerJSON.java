      ...
      try {
        for (int i = 0; i < fields.size(); i += 2) {
          ...
          if (fieldName.equals(ODocumentHelper.ATTRIBUTE_RID))
            ...
            final Object v = OStringSerializerHelper.SKIPPED_VALUE.equals(fieldValue) ?
                new ORidBag() :
                getValue(doc, fieldName, fieldValue, fieldValueAsString, type, null, fieldTypes, noMap, iOptions);
    ...
    if (!iFieldValue.isEmpty()) {
      ...
      for (int i = 1; i < iFieldValue.length(); i++) {
        if (iFieldValue.charAt(i) == ',' || i == iFieldValue.length() - 1) {
          String item = (i == iFieldValue.length() - 1) ?
              iFieldValue.substring(lastCommaPosition + 1) :
              iFieldValue.substring(lastCommaPosition + 1, i);
          lastCommaPosition = i;
