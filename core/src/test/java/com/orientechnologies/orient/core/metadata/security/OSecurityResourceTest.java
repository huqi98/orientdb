...
public class OSecurityResourceTest {
  ...
  public void testParse() {
    ...
    Assert
        .assertEquals("Person", ((OSecurityResourceClass) OSecurityResource.parseResource("database.class.Person")).getClassName());
    ...
    Assert.assertEquals("Person",
        ((OSecurityResourceProperty) OSecurityResource.parseResource("database.class.Person.name")).getClassName());
    Assert.assertEquals("name",
        ((OSecurityResourceProperty) OSecurityResource.parseResource("database.class.Person.name")).getPropertyName());
    ...
    Assert.assertEquals("person",
        ((OSecurityResourceCluster) OSecurityResource.parseResource("database.cluster.person")).getClusterName());
    Assert.assertTrue(OSecurityResource.parseResource("database.function.foo") instanceof OSecurityResourceFunction);
    Assert
        .assertEquals(OSecurityResourceDatabaseOp.BYPASS_RESTRICTED, OSecurityResource.parseResource("database.bypassRestricted"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.COMMAND, OSecurityResource.parseResource("database.command"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.COMMAND_GREMLIN, OSecurityResource.parseResource("database.command.gremlin"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.COPY, OSecurityResource.parseResource("database.copy"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.CREATE, OSecurityResource.parseResource("database.create"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.DB, OSecurityResource.parseResource("database"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.DROP, OSecurityResource.parseResource("database.drop"));
    Assert.assertEquals(OSecurityResourceDatabaseOp.EXISTS, OSecurityResource.parseResource("database.exists"));
