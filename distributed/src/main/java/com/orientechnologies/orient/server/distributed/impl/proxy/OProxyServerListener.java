...
public class OProxyServerListener extends Thread {
  ...
  public OProxyServerListener(final OProxyServer server, final int localPort, final int remotePort) {
    this.server = server;
  ...
  public void run() {
    ...
    try {
      ...
      OLogManager.instance().error(this, "Proxy server: error on creating local socket for proxy channel %d->%s:%d", e, localPort,
          server.getRemoteHost(), remotePort);
    }
