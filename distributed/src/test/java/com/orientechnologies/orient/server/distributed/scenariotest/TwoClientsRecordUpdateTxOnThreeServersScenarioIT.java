...
/**
 * Checks for consistency on the cluster for these steps: - 3 server (server0, server1 and server2) with writeQuorum=2 - a record is
 * inserted on server0 - the record (version 1) is propagated to the other two servers (server1 and server2) - introduce a delay
 * after record locking for server0 and server1 - two clients (connected to server0 and server1) tries to update that record -
 * server0 notifies the update to server1 and server2: server1 return a RecordLockedException, since it's trying to update that
 * record itself as well; server2 returns "true" - server0's request has reached the write quorum and propagates the new value to
