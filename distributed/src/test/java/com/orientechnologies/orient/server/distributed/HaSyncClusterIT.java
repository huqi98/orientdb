...
public class HaSyncClusterIT extends AbstractServerClusterTest {
  ...
  protected void executeTest() throws Exception {
    ...
    ODatabaseDocument db = firstServer.getServerInstance().getContext().open(getDatabaseName(), "admin", "admin");
  ...
  protected Future<Long> invokeSyncCluster(final String localNodeName, final ServerRun server) {
    ...
    Future<Long> future = executorService.submit(new Callable<Long>() {
      ...
      public Long call() throws Exception {
        ...
        ODatabaseDocument db = server.getServerInstance().getContext().open(getDatabaseName(), "admin", "admin");
        try {
          ...
          for (int person : persons) {
            String clusterNameById = ((ODatabaseDocumentInternal) db).getStorage().getPhysicalClusterNameById(person);
